...
const divisionFactor = 0.5; // 180/ my_length
var rad = Math.PI / 180;
...
function updateInput(p, cx, cy, r1, offset, delta) {
  ...
  output.innerHTML = Math.round((a + 180) / divisionFactor);
  initialValue.value = Math.round((a + 180) / divisionFactor);
}
...
window.addEventListener("load", function() {
  var pa = (initVal * divisionFactor) - 180;
  var p = {}
...
initialValue.addEventListener("input", function() {
    var val = this.value;
    var newVal = (!isNaN(val) && val >= 0 && val <= 360) ? val : 18;
    var pa = (newVal * divisionFactor) - 180;
    var p = {}
    p.x = cx + r1 * Math.cos(pa * rad);
    p.y = cy + r1 * Math.sin(pa * rad);
    updateInput(p, cx, cy, r1, offset, delta)
